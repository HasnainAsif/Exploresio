...
router.get("/auth", auth, (req, res) => {
  User.findById(req.user.id)
    ...
    .catch((err) => {
      console.error("errrrrrrrrrrr", err.message);
            ...
            bcrypt.hash(newUser.password, salt, function (err, hash) {
              ...
              mailOptions = {
                ...
                html:
                  ...
                  link +
                  ">Click here to verify</a><br><h3>If it's not you, ignore this message</h3>",
              };
              smtpTransport.sendMail(mailOptions, function (error, response) {
                if (error) {
                  ...
                  console.log(error);
                  res.json({
                    msg: "Error occured in email verification or Check your Internet",
                    alertType: "danger",
                  });
                } else {
                  // console.log("Message sent: " + response.message);
                  res.json({
                    msg: "Please check your email to verify your email address",
                    alertType: "success",
                  });
                }
              });
...
router.get(`/verify`, function (req, res) {
  ...
  if (req.protocol + "://" + req.get("host") == "http://" + host) {
    ...
    User.findOne({ email: userEmail }).then((emailExixts) => {
      if (emailExixts) res.json({ msg: "User already Registered" });
      else {
        newUser
          .save()
          .then((savedUser) => {
    ...
    Newsletter.findOne({ email: userEmail }).then((emailExixts) => {
      if (emailExixts) console.log("Email in Newsletter Already Exists");
      else {
        newNewsletter
          .save()
          .then((saved) => {
      ...
      if (!user) {
        ...
        smtpTransport.sendMail(mailOptions, function (error, response) {
          if (error) {
            console.log(error);
            res.json({
              errors: [{ msg: "Error occured in email verification" }],
            });
          } else {
            //console.log("Message sent: " + response.message);
            res.json({
...
router.get("/update-password", function (req, res) {
  ...
  if (req.protocol + "://" + req.get("host") == "http://" + host) {
    ...
    if (req.query.id == rand) {
      ...
      User.findById(req.query.email_id)
        .select("-password")
        .then((user) => {
          res.json(user);
        })
